...
class TimeSeriesXYSplitter(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None): 
        ...
        if T == self.encode_len: 
            return {
                'X': X[ :, :, :1] , 
                'E': X[ :, :, 1:] if X.shape[2]>1 else None, 
                'y': None
                }     
        else:    
            # extract the X values
            X_adj = X[ :, -( self.encode_len + self.decode_len) :  , :1].copy()     
            # make all values in forecasted horizon -1 so model can 
            # recognize it as different       
            X_adj[ :, -self.decode_len : ] = -1.         
            E = X[ :, -( self.encode_len + self.decode_len) :, 1:] if X.shape[2]>1 else None
            y = X[ :,  -self.decode_len : , :1]
            
            is_backcast = np.ones(shape=(E.shape[0], E.shape[1], 1))
            is_backcast[:, -self.decode_len:, :] = 0
            E = np.append(E, is_backcast, axis=2)            
            return {
                'X': X_adj, 
                'E': E, 
                'y': y
